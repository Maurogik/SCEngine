...
#version 430 core
    ...
    float raymarchTerrainShadow(vec3 start_worldspace, vec3 dir_worldspace, float maxLen, float rayStep)
    {
            ...
            ++i)
        {
            ...
            rayStep *= 1.1 + advanceRatio;
            penumbraSize = advanceRatio * maxPenumbraSize;
            pos_worldspace += dir_worldspace * rayStep;
            float height = terrainHeightAt(pos_worldspace);
            if(pos_worldspace.y <= height)
            {
                return 1.0f;
            }
            softshadow = max(1.0 - min((pos_worldspace.y - height)/penumbraSize, 1.0), softshadow);
        }
        return softshadow;
    }
    ...
    void main()
    {
        ...
        if(occludedByScene > 0.0)
        {
            ...
            pos_worldspace.y = max(pos_worldspace.y, currentHeight);
