...
#version 400 core
    ...
    uniform vec3 WaveMouvement;

    vec3 getNormal(vec2 uv)
    {
        vec2 offset = vec2(ScaleU, ScaleV) * WaveMouvement.xy * WaveMouvement.z * sin(SCE_TimeInSeconds);
        vec3 normal = texture(NormalMap, uv + offset).xyz;
